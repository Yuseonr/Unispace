    ...
    expect(prismaMock.reservation.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          ...
          facilityGroupId: null,
          requestedQuantity: 1,
          reservationNumber: expect.stringMatching(/^RSV-\d{14}-[0-9A-F]{6}$/),
        }),
      }),
    );
...
describe('ReservationsService - listMy', () => {
  ...
  it('tetap menyertakan daftar allocatedAssets jika reservasi sudah COMPLETED', async () => {
    const completedReservation = {
      id: 'res-completed-item',
      userId: stubActiveUser.id,
      facilityId: null,
      facilityGroupId: stubQuantityGroup.id,
      requestedQuantity: 1,
    ...
    prismaMock.reservation.count.mockResolvedValue(1);
    prismaMock.reservation.findMany.mockResolvedValue([completedReservation]);
...
describe('ReservationsService - listStaff & getStaffDetail', () => {
  ...
  it('tetap mengembalikan alokasi aset pada list petugas jika status COMPLETED', async () => {
    prismaMock.reservation.count.mockResolvedValue(1);
    prismaMock.reservation.findMany.mockResolvedValue([
      {
        id: 'res-staff-completed',
        status: ReservationStatus.COMPLETED,
        facility: stubExclusiveFacility,
        items: [],
      },
    ...
    expect(result.data[0].allocatedAssets).toEqual([
      {
        id: stubExclusiveFacility.id,
        assetCode: stubExclusiveFacility.assetCode,
        name: stubExclusiveFacility.name,
      },
    ]);
  });
...
describe('ReservationsService - approve', () => {
  ...
  describe('Mode Ruang Eksklusif', () => {
    ...
    it('tetap menyetujui ruang saat frontend lama mengirim allocatedAssetIds', async () => {
      ...
      const result = await service.approve(
        staffId,
        stubExclusiveRes.id,
        { allocatedAssetIds: ['legacy-value'] },
        now,
      );
  ...
  describe('Mode Kelompok Alat (QUANTITY)', () => {
    ...
    it('memilih unit yang bebas secara deterministik saat body approval kosong', async () => {
      prismaMock.reservation.findUnique
      ...
      expect(prismaMock.facility.findMany).toHaveBeenCalledWith({
        where: {
          ...
          maintenancePeriods: {
            none: {
              startAt: { lt: new Date('2026-09-28T04:00:00.000Z') },
              endAt: { gt: new Date('2026-09-28T02:00:00.000Z') },
            },
          },
          reservationItems: {
            none: {
              reservation: {
                status: ReservationStatus.APPROVED,
                usageDate: stubQuantityRes.usageDate,
                startTime: { lt: stubQuantityRes.endTime },
                endTime: { gt: stubQuantityRes.startTime },
              },
            },
          },
        },
        ...
        take: stubQuantityRes.requestedQuantity,
      });
      ...
      expect(result.allocatedAssets.map((asset) => asset.assetCode)).toEqual([
        'PRJ-001',
        'PRJ-002',
      ]);
    });

    it('mengabaikan allocatedAssetIds dari frontend lama', async () => {
      prismaMock.reservation.findUnique
        .mockResolvedValueOnce(stubQuantityRes)
        .mockResolvedValueOnce({
          ...stubQuantityRes,
      ...
      await service.approve(
        staffId,
        stubQuantityRes.id,
        { allocatedAssetIds: ['frontend-choice-a', 'frontend-choice-b'] },
        now,
      );
    ...
    it('menolak reservasi tanpa membuat item saat stok aktual tidak mencukupi', async () => {
      prismaMock.reservation.findUnique
        .mockResolvedValueOnce(stubQuantityRes)
        .mockResolvedValueOnce({
          ...stubQuantityRes,
      ...
      expect(prismaMock.reservationItem.createMany).not.toHaveBeenCalled();
      expect(prismaMock.reservation.updateMany).toHaveBeenCalledWith({
        where: {
    ...
    it('tidak membuat item atau audit approval kedua saat status sudah diproses', async () => {
      ...
      await expect(
        service.approve(staffId, stubQuantityRes.id, {}, now),
      ).rejects.toThrow('Reservasi sudah diproses oleh petugas lain.');
    ...
    it('menyetujui reservasi alat, mencatat ReservationItem, dan cascade auto-reject pengajuan PENDING yang kekurangan stok', async () => {
      ...
      expect(prismaMock.reservation.updateMany).not.toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ id: 'res-pending-surplus' }),
        }),
      );
...
describe('ReservationsService - reject', () => {
  ...
  it('menolak reservasi PENDING, menyimpan alasan, dan mencatat audit log', async () => {
    ...
    expect(prismaMock.reservation.updateMany).toHaveBeenCalledWith({
      where: { id: stubPendingRes.id, status: ReservationStatus.PENDING },
      data: {
        status: ReservationStatus.REJECTED,
        decisionReason: 'Ruangan sedang dalam persiapan acara wisuda.',
        processedById: staffId,
...
describe('ReservationsService - cancelByStaff', () => {
  ...
  it('melemparkan BadRequestException jika status reservasi bukan APPROVED', async () => {
    prismaMock.reservation.findUnique.mockResolvedValue({
  ...
  it('membatalkan reservasi APPROVED oleh staf, mencatat cancelledAt dan audit log', async () => {
    ...
    expect(prismaMock.reservation.updateMany).toHaveBeenCalledWith({
      where: { id: stubApprovedRes.id, status: ReservationStatus.APPROVED },
      data: {
        status: ReservationStatus.CANCELLED_BY_STAFF,
        decisionReason:
          'Ruangan dialihkan mendadak untuk agenda kunjungan rektorat.',
  ...
  it('menolak pembatalan reservasi PENDING oleh staf karena harus memakai reject', async () => {
    const stubPendingRes = {
      id: 'res-cancel-pending',
      status: ReservationStatus.PENDING,
      facilityId: stubExclusiveFacility.id,
    ...
    prismaMock.reservation.findUnique.mockResolvedValue(stubPendingRes);

    await expect(
      service.cancelByStaff(
        staffId,
        stubPendingRes.id,
...
describe('ReservationsService - completeFinishedReservations', () => {
  let service: ReservationsService;
  ...
  it('mengubah reservasi APPROVED yang sudah selesai menjadi COMPLETED sekali', async () => {
    ...
    const count = await service.completeFinishedReservations(now);
  ...
  it('tidak mengubah reservasi yang waktu selesai belum tercapai', async () => {
...
describe('ReservationsService - autoRejectExpiredReservations & SLA enforcement', () => {
  ...
  it('menolak seluruh reservasi PENDING yang melewati decisionDeadline secara transaksional', async () => {
    ...
    expect(count).toBe(2);
    expect(prismaMock.reservation.updateMany).toHaveBeenNthCalledWith(1, {
      where: {
        id: 'res-expired-1',
        status: ReservationStatus.PENDING,
    ...
    expect(prismaMock.reservation.updateMany).toHaveBeenNthCalledWith(2, {
      where: {
        id: 'res-expired-2',
        status: ReservationStatus.PENDING,
        decisionDeadline: { lte: now },

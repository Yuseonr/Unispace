...
export function FacilityEditor({ facilityId }: { facilityId?: string }) {
  ...
  const [pendingUnitToggle, setPendingUnitToggle] = useState<AdminFacilityUnit | null>(null);
  ...
  return (
    <main className="admin-page admin-facility-editor-page">
      ...
      <ConfirmDialog busy={Boolean(pendingUnitToggle && savingUnitId === pendingUnitToggle.id)} confirmLabel={pendingUnitToggle?.status === "ACTIVE" ? "Nonaktifkan unit" : "Aktifkan unit"} destructive={pendingUnitToggle?.status === "ACTIVE"} error={pendingUnitToggle ? error : null} isOpen={Boolean(pendingUnitToggle)} onClose={() => { if (!savingUnitId) setPendingUnitToggle(null); }} onConfirm={() => { if (pendingUnitToggle) void toggleUnit(pendingUnitToggle).then((success) => { if (success) setPendingUnitToggle(null); }); }} title={pendingUnitToggle?.status === "ACTIVE" ? "Nonaktifkan unit aset" : "Aktifkan unit aset"}>{pendingUnitToggle ? <p>{pendingUnitToggle.status === "ACTIVE" ? <>Unit <strong>{pendingUnitToggle.assetCode}</strong> tidak akan dapat dipesan. Bila masih ada reservasi disetujui, backend akan menolak perubahan dan menjelaskan alasannya.</> : <>Aktifkan kembali unit <strong>{pendingUnitToggle.assetCode}</strong> agar dapat masuk ke ketersediaan alat?</>}</p> : null}</ConfirmDialog>
    </main>
  );
}
